...
import { shouldCompactScreenshots } from '@/lib/onepageLimits'
...
export default function OnepagePage() {
  ...
  const compactMedia = shouldCompactScreenshots(project, team.team_declaration ?? '')
        ...
        <header className="flex items-start justify-between mb-3">
          ...
          <div className="text-right shrink-0 pl-2 max-w-[48%]">
            <div className="flex justify-end mb-1">
              <BrandLogo className="h-7 w-auto max-w-[140px] object-left" />
            </div>
            <p className="text-[7px] text-green-primary/70 font-mono mt-0.5">RED HACKATHON</p>
          </div>
        </header>
        ...
        {hasMedia && (
          <div className="flex gap-3 mt-1">
            {screenshots.length > 0 && (
              <div className="flex-1 min-w-0">
                <SectionLabel title="关键效果截图" />
                <div className={`grid gap-2 ${screenshots.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
                  {screenshots.map((url, idx) => (
                    <div
                      key={idx}
                      className="rounded-lg border border-green-primary/30 bg-black/40 flex items-center justify-center p-1.5 min-h-0"
                    >
                      <img
                        src={url}
                        alt={`截图 ${idx + 1}`}
                        className={`max-w-full w-auto h-auto object-contain ${compactMedia ? 'max-h-[132px]' : 'max-h-[200px]'}`}
                      />
                    </div>
                  ))}
                </div>
            ...
            {demoQr.length > 0 && (
              <div className={`shrink-0 ${compactMedia ? 'w-[80px]' : 'w-[92px]'}`}>
                <SectionLabel title="Demo 二维码" />
                <div className="border border-green-primary/35 rounded-lg p-1.5 inline-block bg-white/10">
                  <img
                    src={demoQr}
                    alt="Demo QR"
                    className={`object-contain ${compactMedia ? 'w-[72px] h-[72px]' : 'w-20 h-20'}`}
                  />
                </div>
              </div>
            )}
...
function SectionLabel({ title }: { title: string }) {
  return (
    <h3 className="text-sm font-semibold text-green-bright uppercase tracking-wider mb-1.5 font-mono">
      {title}
    </h3>
  )
}
